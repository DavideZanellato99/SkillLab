...
Serve a rallentare chi prova password a raffica sull'endpoint di login.
...
``audit``, e per lo stesso motivo: il tentativo fallito va registrato anche
quando la richiesta che lo ha prodotto finisce con un errore, e la sessione
di quella richiesta non arriverà mai a un commit.

Le righe contengono un indirizzo IP, quindi vengono buttate presto: quelle
fuori finestra spariscono mentre si contano i fallimenti, e ``purge_expired``
raccoglie il resto dallo sweep di ``housekeeping``.
"""
...
class SlidingWindowLimiter:
    """Conta i fallimenti di una chiave su una finestra che scorre."""

    def __init__(self, scope: str, max_failures: int, window_seconds: int):
    ...
    def retry_after(self, key: str) -> int:
        ...
        with session_factory() as db:
            ...
            if len(falliti) < self.max_failures:
                return 0
            # Un posto si libera quando il fallimento più vecchio esce dalla
            # finestra.
            libero_fra = (falliti[0] + timedelta(seconds=self.window_seconds)) - now
...
def purge_expired(conn: Connection) -> int:
    """Elimina i tentativi fuori da qualunque finestra ancora in uso.
