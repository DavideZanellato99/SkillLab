...
def test_allows_until_the_limit_is_reached(db_session):
    limiter = SlidingWindowLimiter(scope="ip", max_failures=3, window_seconds=60)
...
def test_reset_clears_the_key(db_session):
    limiter = SlidingWindowLimiter(scope="ip", max_failures=1, window_seconds=60)
    limiter.record_failure("ip")
    assert limiter.retry_after("ip") > 0
    limiter.reset("ip")
    assert limiter.retry_after("ip") == 0
...
def test_expired_failures_leave_the_window(db_session):
    ...
    limiter = SlidingWindowLimiter(scope="ip", max_failures=1, window_seconds=0)
    limiter.record_failure("ip")
...
def test_the_two_buckets_do_not_count_each_other(db_session):
    ...
    per_email = SlidingWindowLimiter(scope="email", max_failures=1, window_seconds=60)
    per_ip = SlidingWindowLimiter(scope="ip", max_failures=1, window_seconds=60)

    per_email.record_failure("stessa-chiave")
...
def test_recording_a_failure_drops_what_left_the_window(db_session):
    ...
    aspettare nessuno."""
    limiter = SlidingWindowLimiter(scope="ip", max_failures=5, window_seconds=60)
...
def test_the_sweep_leaves_the_attempts_still_being_counted(db_session):
    ...
    limiter = SlidingWindowLimiter(scope="ip", max_failures=1, window_seconds=15 * 60)
    limiter.record_failure("203.0.113.7")
...
def test_two_replicas_share_one_bucket(db_session):
    ...
    replica_uno = SlidingWindowLimiter(scope="ip", max_failures=3, window_seconds=60)
    replica_due = SlidingWindowLimiter(scope="ip", max_failures=3, window_seconds=60)

    replica_uno.record_failure("203.0.113.7")
    replica_uno.record_failure("203.0.113.7")
    replica_due.record_failure("203.0.113.7")
