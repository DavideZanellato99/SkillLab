...
| `simulation_screen_recordings` | Lo schermo registrato durante un test tecnico, una riga per tentativo. Stessa regola dell'audio: la colonna del blob è `deferred` e sta in `STORAGE EXTERNAL` |
...
| `avatar_requests`                                                                           | La richiesta di un avatar nuovo mandata da un organization admin: i pochi campi da cui la scheda nasce, lo stato, e l'avatar che ne è nato o il motivo del rifiuto. Vedi [avatar-e-persona.md](avatar-e-persona.md)                                                                                                                                             |
...
| `technical_simulations`, `simulation_chunks`, `simulation_questions`, `simulation_attempts` | Il simulatore tecnico, descritto in [simulatore.md](simulatore.md). Sulla prima stanno anche l'esito dell'ultimo controllo del serbatoio, l'impronta delle domande su cui è girato e la spunta che registra lo schermo; sull'ultima, se quel tentativo doveva arrivare con la registrazione                                                                                                                                                                             |
...
| `path_debriefings`                                                                          | I quadri d'insieme su un percorso, una riga per ogni volta che ne è stato chiesto uno, ciascuna con la fotografia delle tappe di allora e con l'impronta del gruppo che le permette di dire se sta confrontando le stesse persone. Parla del gruppo e non nomina nessuno, quindi non contiene dati personali. Vedi [training-e-report.md](training-e-report.md) |
