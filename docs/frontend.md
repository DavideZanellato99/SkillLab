...
| `/app/confronto`                                                                             | autenticato | Confronto fra i propri tentativi (per un admin, quelli di una persona del proprio tenant, più una seconda sezione con le medie di più persone a confronto) |
...
| `/app/admin/dashboard/*`, `/app/admin/training`, `/app/admin/report`, `/app/admin/simulations` | admin       | Le tre viste della dashboard, percorsi a tappe, report per utente, test tecnici           |
...
tre viste stanno sotto di lui, una per rotta e una per file scaricato
(vedi [training-e-report.md](training-e-report.md)). Sono tre domande
...
**Un click fuori chiude, ovunque sia.** Lo fa
[useCloseOnClickOutside](../frontend/src/hooks/useCloseOnClickOutside.ts), che
ascolta il `pointerdown` sul documento e chiude quando il punto toccato non
sta né nel pannello né nel pulsante che lo apre: il pulsante deve restare
quello che richiude, e chiuderlo dall'hook lo farebbe riaprire subito dopo al
click. Prima c'erano tre veli trasparenti stesi sotto la barra, e non
funzionavano: la barra sfoca quello che le passa dietro, e un elemento con
`backdrop-filter` diventa il riferimento dei propri discendenti `fixed`, che si
misurano su di lui e non sulla finestra. Un velo che doveva andare dalla base
della barra al fondo dello schermo era alto zero, e il menu del profilo restava
aperto finché non lo si richiudeva dal pulsante. Anche coprissero la pagina, i
veli lascerebbero fuori la barra, e un click sul logo o sullo spazio accanto
non chiuderebbe niente. Il pannello delle sezioni tiene il proprio velo solo
per scurire la pagina, e per la stessa ragione lo manda in fondo al documento
da un portal.
...
**Ogni voce che quel ruolo vede ha il suo passo.** È la regola che tiene
insieme i due elenchi con `navEntries`: chi si allena attraversa le cinque
sezioni in barra, le notifiche e il proprio account, chi amministra le quattro
in barra, le tre nel menu del profilo e l'account. Le notifiche sono l'unica
eccezione, e sta dal lato del server: `notifications.for_user` le ricava dai
propri percorsi e dalle revisioni ricevute, cose che un amministratore non ha,
quindi la sua campanella non ha niente da annunciare e la sua guida non ne
parla. Una sezione nuova va aggiunta qui insieme alla voce, altrimenti resta in
barra senza che la guida dica cosa sia.
...
| [DataTable](../frontend/src/components/DataTable.tsx)                                                                                                                                                        | Tabella, intestazione, righe e ricerca. Sfogliare le righe non è affare suo, sta in `Pagination`. Le colonne stanno a misure fisse: ogni colonna dichiara la propria `width` in percentuale (obbligatoria, e le percentuali di una tabella sommano a 100), il layout è `table-fixed` e sotto `minWidth` scorre il riquadro invece di stringersi le colonne. Anche le righe del corpo stanno a un'altezza fissa, una per tutte le tabelle ([rowHeights.ts](../frontend/src/components/rowHeights.ts), 80px): due righe del testo delle celle più l'imbottitura. Ogni riga la prende, così quella con meno dentro resta alta come le altre con il contenuto centrato. Prima le righe di una stessa tabella non erano tutte alte uguali, una persona con l'email sotto il nome più di una senza, e sfogliando le pagine la tabella cambiava altezza e con lei tutto quello che stava sotto. La misura viaggia in una variabile CSS sulla tabella e vale per le sole righe dell'elenco: intestazione e fasce in fondo hanno la loro, e la riga che si apre sotto un'altra si dichiara con `detail` su `Tr`, che la tiene fuori dall'altezza fissa e dall'evidenziazione al passaggio, perché è un pannello e non una riga. L'altezza regge perché il testo va a capo una volta sola: quando la cella contiene testo e basta, `Td` lo chiude da sé in `line-clamp-2`, e quello che non entra nemmeno sulla seconda riga si tronca con i puntini; quando contiene una composizione (il nome con l'email sotto, la targhetta con il titolo accanto) il nome sta su una riga con `truncate` e la riga sotto pure, e la cella tiene comunque il contenuto entro l'altezza della riga, così un pezzo che si dimentica del limite non la sfonda. Un flex mette `min-w-0` sul testo perché possa stringersi. Il nome di una persona, di un avatar, di una simulazione o di una organizzazione sta su una riga sola: è corto per natura e la seconda riga la spende quello che gli sta sotto; quello che avrebbe bisogno di una terza riga si mette accanto (l'organizzazione accanto all'email nelle assegnazioni dei percorsi, per il super admin). C'erano tre misure nominate per contenuto, e una dopo l'altra le tabelle sono finite tutte nella più bassa, perché una riga più alta serviva sempre a un a capo che non arrivava mai. Il tooltip con il testo intero lo porta la cella, non chi la disegna: al passaggio del mouse `Td` guarda cosa dentro di sé è stato tagliato (la regola sta in [truncation.ts](../frontend/src/components/truncation.ts), la stessa di `truncateOnly`) e lo mostra per intero, una riga per pezzo, mentre su una cella che si legge tutta non compare niente e il tooltip della riga resta suo. Un pezzo che dentro la cella porta già un tooltip proprio, come il registro di un'operazione, viene lasciato a lui. Prima ogni cella con un testo che poteva non entrare doveva ricordarsi di avvolgerlo, e quelle che non se lo ricordavano tagliavano senza dire cosa. Riceve i dati (`items`) e come si disegna una riga (`renderRow`), non le righe già costruite: finché arrivavano come `children`, la pagina costruiva un albero JSX per ogni elemento dell'elenco e la tabella ne mostrava venti, quindi su un report di tremila valutazioni se ne buttavano via duemilanovecentottanta a ogni battuta scritta nella ricerca. `renderRow` restituisce un elemento con la propria `key`, un `<Tr>` o un Fragment quando sotto la riga se ne apre una seconda. Lo stato vuoto non si dichiara più: deriva da `items`. Intestazioni e celle sono centrate nella propria colonna, e una cella che dentro si costruisce con un flex lo centra con `justify-center`. Una riga che apre qualcosa lo dichiara con `onActivate` invece di un `onClick` scritto a mano: da lì arrivano insieme il puntatore a manina, il fuoco da tastiera e Invio o Spazio per aprirla, che erano tre cose da ricordarsi ogni volta e che infatti mancavano, lasciando il dettaglio di una conversazione e di un test raggiungibili col solo mouse. La riga resta una riga e non diventa un bottone: `role="button"` le toglierebbe il posto nella griglia proprio nelle tabelle che si aprono. Le eccezioni le dichiara la cella con `align="left"`, e l'intestazione resta comunque al centro: la prima colonna della gestione utenti, degli avatar, del report attività, dei percorsi assegnati e delle due tabelle della dashboard, cioè quella che dà il nome alla riga, dove un'immagine o una targhetta seguita da un nome si scorre con l'occhio, e i pannelli che si aprono sotto una riga, che sono elenchi di voci e valori. `footerNote` è una fascia in fondo alla scheda, sotto la barra per sfogliare, per gli elenchi che dal server arrivano a finestre: dice quante righe sono state scaricate sul totale e offre di chiederne altre. Sta lì dentro e non sotto la tabella perché due conteggi sulle stesse righe a un centimetro di distanza si leggono come una contraddizione, e nella stessa striscia si vede che uno conta quello che si sta guardando e l'altro quello che è arrivato. `pageResetKey` è cosa rende queste righe un elenco diverso, i filtri attivi di solito: quando cambia si torna alla prima pagina, e l'ordinamento ci finisce da sé. Ordinare è della tabella e ha due modi. Dove i dati sono tutti in memoria la colonna dichiara `sortValue`, cioè come si legge da una riga il valore su cui ordina, e la tabella fa il resto: confronto con un `Intl.Collator` costruito una volta sola (`localeCompare` riga per riga rimette insieme le regole della lingua a ogni coppia, e su un elenco lungo è il grosso del tempo), celle vuote in fondo in tutti e due i versi perché una cella senza valore non è né la più piccola né la più grande, e ritorno alla prima pagina quando l'ordine cambia. Dove l'elenco arriva a finestre dal server, cioè la gestione utenti e il registro attività, la tabella non ordina niente: riceve `sort` e `onSortChange`, disegna l'intestazione attiva e riporta la scelta, perché ordinare qui vorrebbe dire ordinare le duecento righe già scaricate e chiamarle le prime duecento di tutte. Lì la colonna si dichiara ordinabile con `sortable`, e le colonne che il server sa ordinare stanno in `USER_SORT_COLUMNS` e `AUDIT_SORT_COLUMNS`, con le stesse chiavi da una parte e dall'altra. Il clic sull'intestazione gira su tre stati e non su due, crescente, decrescente e nessun ordinamento, in tutti e due i modi: anche l'ordine di arrivo è una risposta, quello in cui l'elenco è stato messo in fila da chi lo ha prodotto (il cognome in ordine alfabetico nella gestione utenti, la riga più recente per prima nel registro attività), e senza il terzo clic lo si recupererebbe solo ricaricando la pagina. Sulle tabelle che ordinano lato server il terzo clic riporta `null`, cioè nessuna chiave nella richiesta e il default del router. L'intestazione ordinabile è un `<button>` a tutta cella, perché è un comando e deve arrivare col tabulatore e con Invio, e `aria-sort` sta sulla cella, dove lo standard lo cerca |
...
| [Tooltip](../frontend/src/components/Tooltip.tsx)                                                                                                                                                            | Ogni spiegazione al passaggio del mouse. Vive in un portal, quindi non lo taglia il bordo di una tabella o di una modale, e di suo non aggiunge nodi al DOM: clona il figlio e gli aggancia gli eventi. Con `truncateOnly` compare solo se quel testo è davvero tagliato, su una riga (`.truncate`) o su più righe (`line-clamp-*`): su un testo intero ripeterebbe parola per parola quello che si sta già leggendo. Il taglio lo cerca anche nei testi dentro l'elemento, così il tooltip si può agganciare al riquadro che li contiene invece che a ogni riga, e a rispondere al mouse è tutta l'area che si passa sopra. Uno con `truncateOnly` che non ha niente da dire non zittisce il tooltip più esterno (la cella di una tabella dentro la riga che dice «Vedi il test svolto»), e quando ne stanno tre uno dentro l'altro il più interno spegne tutti quelli sopra, non solo il suo vicino. Quello che avvolge riceve il segno `data-tooltip`, così un tooltip più esterno sa quale pezzo lasciare a lui                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| [TableSkeleton](../frontend/src/components/TableSkeleton.tsx)                                                                                                                                                | La forma della tabella mentre le righe stanno arrivando, al posto della rotella centrata nelle sei pagine che aprono un elenco. Prende le stesse colonne della tabella vera e la stessa altezza di riga, quindi le intestazioni sono già al loro posto e le celle grigie stanno alle misure che avranno, in larghezza e in altezza: la pagina non resta vuota e poi si riempie di colpo spostando in basso tutto quello che sta sotto. Cinque righe finte, meno di quante ne arriveranno, perché uno scheletro più lungo dell'elenco vero farebbe rimpicciolire la scheda proprio quando i dati compaiono. Per chi non lo vede resta quello di prima: il contenitore fa da `role="status"` e porta la frase, le celle finte sono `aria-hidden`. La dashboard tiene la rotella, perché lì quello che arriva sono grafici e uno scheletro di tabella al loro posto direbbe la cosa sbagliata                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| [GalleryHero](../frontend/src/components/GalleryHero.tsx) e i pezzi delle gallerie                                                                                                                           | L'impianto delle due gallerie, quella degli avatar e quella dei test tecnici: la testata con i numeri e lo slot per un'azione nell'angolo in alto a destra (`GalleryHero`), le misure della griglia e il ritardo a cascata delle tessere (`galleryLayout`), i segnaposto che ne prendono il posto mentre si aspetta (`GallerySkeleton`), il riquadro del vuoto con il gesto che lo rimedia (`GalleryEmpty`) e l'avviso di un rinfresco caduto su un elenco che resta a schermo (`StaleDataToast`). Sono le due schermate da cui si sceglie cosa fare adesso, si aprono dalla stessa barra e si scorrono con la stessa domanda in testa: tenerne due copie voleva dire due gallerie che a un certo punto non si somigliano più, con le colonne che cambiano larghezza e l'avviso che si sposta passando dall'una all'altra. Quello che resta scritto in ciascuna è la materia, cioè le parole della testata, cosa c'è sulle tessere e le tre ragioni per cui una griglia può essere vuota                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| [Field](../frontend/src/components/Field.tsx), [Select](../frontend/src/components/Select.tsx), [SearchSelect](../frontend/src/components/SearchSelect.tsx)                                                  | I campi dei form, con le classi già decise. `SearchSelect` ha due varianti. Sopra (`above`, il default) la chip, o il testo muto quando non c’è scelta, sta fuori dal flusso a destra sulla riga dell’etichetta e il campo resta largo sempre uguale: è quella del campo accanto al titolo del confronto tra tentativi e del filtro utente della dashboard, lo stesso posto del contatore di `MultiSearchSelect` nell’altra sezione del confronto. Come campo di un form (`variant="field"`) la chip prende il posto del campo, che torna quando si toglie la scelta. C’era una terza variante con la chip accanto al campo: ogni scelta lo restringeva, e un campo che cambia larghezza alla prima scelta si legge come un altro campo; sotto non poteva stare, perché i suggerimenti aperti l’avrebbero coperta. I nomi non si tagliano mai: l'elenco dei suggerimenti si allarga quanto il nome più lungo invece di stare nella larghezza del campo, e come campo di un form il nome scelto va a capo. Chi cerca sta scegliendo fra cose che si somigliano, e spesso si distinguono per l'ultima parola. [MultiSearchSelect](../frontend/src/components/MultiSearchSelect.tsx) è il gemello a più scelte, per il confronto fra utenti nella pagina del confronto: stesso stile e stesso pattern ARIA, ma la lista resta aperta dopo ogni spunta (chi compone un confronto ne sceglie tre o quattro di fila) e le voci scelte restano marcate in elenco, da dove si tolgono ricliccandole. Con `selectAllLabel` la lista comincia con una riga che spunta tutte le voci in una volta, spuntata quando lo sono tutte, e che compare solo a ricerca vuota. La lista sta in un portal a posizione fissa, come il menu del `KebabMenu`: la scheda in cui il campo è nato aveva un `backdrop-blur`, che è un contesto di impilamento a sé, e con poche barre sotto il campo la lista finiva sotto la scheda successiva. Si rimisura anche con un `ResizeObserver` sulla radice del documento e sul blocco, perché ogni spunta allunga la pagina, fa comparire la barra di scorrimento e sposta il contenuto centrato di qualche pixel senza nessun evento di scroll o resize. Non porta targhette con i nomi scelti: chi lo usa comanda qualcosa che si vede subito sotto, e riscrivere lì gli stessi nomi sarebbe la stessa cosa detta due volte a un centimetro di distanza. Sopra il campo, a destra sulla riga dell’etichetta e fuori dal flusso, stanno quante sono e «Azzera», che è l’unica cosa che dall’elenco non si fa in un gesto solo: accanto al campo lo restringevano, sotto la lista aperta li copriva, e in flusso avrebbero spinto giù il campo alla prima spunta. Le voci di una tendina di persone stanno in ordine alfabetico per cognome, con la regola della tabella di gestione utenti scritta una volta in [personOrder](../frontend/src/components/personOrder.ts), da cui viene anche il `Intl.Collator` che ordina le tabelle: due elenchi delle stesse persone ordinati in due modi si leggono come due elenchi diversi                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| [PasswordField](../frontend/src/components/PasswordField.tsx) e [PasswordRules](../frontend/src/components/PasswordRules.tsx)                                                                                | I campi in cui si sceglie una password, e l'elenco dei requisiti che si accendono man mano. Sono i sei campi della modale di accesso e della pagina del profilo: il bottone occhio è del campo e non del modulo che lo ospita, e si richiude da solo quando il campo si svuota, altrimenti dopo un cambio password riuscito la password successiva comparirebbe in chiaro a chi non ha chiesto di vederla. Nei due posti in cui si sceglie una password, la modale di accesso e la pagina del profilo, l'elenco sta sotto la conferma e prende in fondo la riga sulla coincidenza fra le due password: è l'ultima cosa da sistemare prima di inviare, quindi è l'ultima riga sotto l'ultimo campo, e in entrambi il bottone resta spento finché l'elenco non è tutto verde, senza nessun avviso che ripeta la stessa cosa                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
- **Un comportamento con uno stato suo diventa un hook**, non un altro gruppo
  ...
  stesso ritardo in ogni casella che cerca), il trascinamento di un elemento
  da una zona all'altra
  ([usePointerDrag](../frontend/src/hooks/usePointerDrag.ts), gli eventi del
  puntatore e non il drag and drop del browser, che su un telefono non parte
  affatto: le zone si dichiarano nel DOM con `data-drop-zone`, il fantasma lo
  disegna chi usa l'hook, e un movimento più corto di qualche pixel resta un
  click, così chi non trascina può toccare).
...
- **Le regole del React Compiler valgono anche senza il compiler.** `oxlint`
  le applica (`react/set-state-in-effect`, `react/refs`, `react/purity`) e il
  codice le rispetta, non le silenzia. In pratica sono tre abitudini. Uno
  stato che si azzera quando cambia una prop si azzera durante il render, con
  lo stato «visto l'ultima volta» accanto (la pagina uno di
  [usePagination](../frontend/src/hooks/usePagination.ts) al cambio dei
  filtri, il menu di [Navbar](../frontend/src/components/Navbar.tsx) al cambio
  di pagina, la copia locale delle domande in
  [SimulationEditorModal](../frontend/src/components/SimulationEditorModal.tsx)),
  non in un `useEffect` dopo: l'effetto costa un render con la cosa vecchia a
  schermo. Un ref non si legge e non si scrive durante il render: un valore
  che serve nel render è uno stato, anche se non cambia mai (l'istante di
  partenza del cronometro in
  [SimulationQuestionStep](../frontend/src/components/SimulationQuestionStep.tsx),
  con l'inizializzatore pigro di `useState`), e un oggetto che contiene un ref
  viene trattato tutto come ref, per cui `inputRef` viaggia come prop a sé e
  non dentro l'oggetto `chat` di [ChatDock](../frontend/src/components/ChatDock.tsx).
  Un callback che un effetto deve chiamare nell'ultima versione, senza
  ripartire a ogni cambio, è un `useEffectEvent`
  ([useIdleLogout](../frontend/src/hooks/useIdleLogout.ts),
  [usePointerDrag](../frontend/src/hooks/usePointerDrag.ts)), non un ref
  riscritto a ogni render; dove il chiamante non è un effetto ma un client
  esterno ([VoiceButton](../frontend/src/components/VoiceButton.tsx)) il ref
  resta, ma si aggiorna in un effetto.
...
- **Le tendine di una barra di filtri stanno nell'ordine delle colonne che
  ...
  colonna; nella gestione simulazioni l'organizzazione apre la fascia come
  apre la tabella, poi il tipo e l'origine, che sono le due targhette di una
  colonna sola, e infine lo stato. Chi cerca il comando che restringe una
  colonna lo trova sopra la colonna che sta guardando, e una barra ordinata a
  caso lo fa leggere tutto ogni volta. Un filtro senza colonna (lo stato degli
  avatar, che in tabella non c'è) va in fondo, e le due date del registro
  restano in fondo anche loro: sono un intervallo che si sceglie una volta,
  non una tendina che si cambia di continuo.
...
- **Le formattazioni condivise stanno in un modulo a parte** accanto ai
  ...
  [numberFormat.ts](../frontend/src/components/numberFormat.ts),
...
- **Un voto ha due colori in tutta l'app: verde dal sei, arancione sotto.** La
  soglia è `PASS_SCORE` in [simulationFormat.ts](../frontend/src/components/simulationFormat.ts),
  che colora la targhetta dei test scritti (`scoreBadgeTone`), e la leggono le
  funzioni di [scoreFormat.ts](../frontend/src/components/scoreFormat.ts) per
  il testo e le barre (`scoreTextColor`, `scoreBarColor`): le pagelle, le
  tabelle e le medie della dashboard, il confronto, i report dei percorsi.
  Niente rosso e niente terza fascia: prima c'erano due scale diverse, una a
  tre tinte con soglie a 5 e 7 nelle pagelle e nella dashboard, una a tre
  tinte con soglie a 6 e 8 nel simulatore, e lo stesso sette era verde in
  una schermata e ambra in quella accanto. Il rosso resta agli errori, alle
  azioni distruttive e alla variazione in peggio fra due prove, che è una
  direzione e non un voto.
...
- **Il formattatore si costruisce una volta sola, mai dentro la funzione che
  scrive.** `toLocaleDateString` e `toLocaleString` chiamati con delle opzioni
  costruiscono un `Intl` nuovo a ogni chiamata, ed è la costruzione a costare,
  non la scrittura: sta come costante di modulo, e i momenti del server ci
  passano attraverso con `formatInstant`
  ([instant.ts](../frontend/src/components/instant.ts)), che dà anche il
  trattino a una data illeggibile invece di lasciare sollevare un errore
  dentro un render. È la stessa ragione per cui l'`Intl.Collator` che ordina
  le tabelle è costruito una volta in [personOrder](../frontend/src/components/personOrder.ts):
  erano ventuno punti che lo rifacevano a ogni cella, cioè una costruzione per
  riga per colonna nelle tabelle del registro attività e dei report.
