...
| [Field](../frontend/src/components/Field.tsx), [Select](../frontend/src/components/Select.tsx), [SearchSelect](../frontend/src/components/SearchSelect.tsx)                                                  | I campi dei form, con le classi già decise. Un'opzione di `Select` può portare una `note`, un testo secondario accanto all'etichetta per una scelta che resta possibile ma che vale la pena segnalare: la riga si attenua e basta, non si disabilita. La usa l'abbinamento del simulatore, dove ogni tendina dice quali abbinati sono già andati a un'altra voce e a quale. `SearchSelect` ha due varianti: come filtro la scelta sta in una chip accanto al campo di ricerca, come campo di un form (`variant="field"`) la chip prende il posto del campo, che torna quando si toglie la scelta. I nomi non si tagliano mai: l'elenco dei suggerimenti si allarga quanto il nome più lungo invece di stare nella larghezza del campo, e come campo di un form il nome scelto va a capo. Chi cerca sta scegliendo fra cose che si somigliano, e spesso si distinguono per l'ultima parola. [MultiSearchSelect](../frontend/src/components/MultiSearchSelect.tsx) è il gemello a più scelte, per il confronto fra utenti della dashboard: stesso stile e stesso pattern ARIA, ma la lista resta aperta dopo ogni spunta (chi compone un confronto ne sceglie tre o quattro di fila) e le voci scelte restano marcate in elenco, da dove si tolgono ricliccandole. Non porta targhette con i nomi scelti: chi lo usa comanda qualcosa che si vede subito sotto, e riscrivere lì gli stessi nomi sarebbe la stessa cosa detta due volte a un centimetro di distanza. Accanto al campo resta il solo «Azzera», che è l'unica cosa che dall'elenco non si fa in un gesto solo. Le voci di una tendina di persone stanno in ordine alfabetico per cognome, con la regola della tabella di gestione utenti scritta una volta in [personOrder](../frontend/src/components/personOrder.ts), da cui viene anche il `Intl.Collator` che ordina le tabelle: due elenchi delle stesse persone ordinati in due modi si leggono come due elenchi diversi                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
