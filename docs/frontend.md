...
È montata sempre, ed è da lì che si passa da una sezione all'altra. Sta in sette
...
**Ne resta aperto uno solo.** Il pannello delle sezioni e il menu del profilo
escono dallo stesso angolo e sovrapposti sarebbero illeggibili, quindi lo stato
è uno («quale menu è aperto») e sta nella barra, non due interruttori nei due
componenti: aprire l'uno chiude l'altro.

I due menu si chiudono con Esc, dichiarano `aria-expanded` e segnano con
`aria-current` la pagina in cui si è. Esc passa da
...
modali ce l'hanno già dentro `ModalShell`).
...
| [LoadError](../frontend/src/components/LoadError.tsx)                                                                                                                                                        | Il banner di un caricamento caduto con accanto il comando per richiederlo, che è l’unico errore a cui si può rimediare restando dove si è: senza, dentro una modale l’unica via è ricaricare la pagina e riaprire quello che si stava leggendo. Mentre il nuovo tentativo è in corso il bottone lo dice e si blocca. Era ricopiato a mano, riquadro rosso e gradiente compresi, nel dettaglio di una conversazione e nella valutazione di una chiamata |
