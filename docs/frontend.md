...
`admin` e `super admin` della tabella sono import dinamici, il resto no: sono
otto schermate dense di tabelle e modali, e con un import normale finivano nel
primo file che il browser scarica, addosso anche a chi apre l'applicazione solo
per fare una telefonata di prova. Quali siano sta in
[lazyPages.ts](../frontend/src/components/lazyPages.ts), che le tiene in un
elenco solo perché le chiede anche la barra (vedi sotto). Il confine è quello
dei permessi, non una misura di comodo: se una schermata admin viene usata
anche altrove, quel pezzo va estratto in un file suo (è il caso di
...
arrivare. Vale per ogni collegamento che porta a una di quelle pagine, non
solo per la barra: la scheda di un'organizzazione, che apre i suoi utenti e i
suoi avatar, monta lo stesso precarico. Serve perché l'attesa, senza, non si vedrebbe nemmeno: React Router
