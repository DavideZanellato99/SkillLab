...
export default function ChatPage() {
  ...
  // Keep (or drop) each bubble's DOM node in the lookup used to scroll to a
  // cited message; MessageBubble hands the node up through here.
  ...
  return (
    <div
      className="flex h-[calc(100vh-4rem)] animate-fade-in overflow-hidden [animation-duration:0.3s]"
      id="chat-page"
    >
      {/* Sidebar */}
      <aside
                ...
                return (
                  <li
                    ...
                    className={`group/conv flex cursor-pointer items-center gap-2 rounded-lg p-2 transition ${
                      isActive
                        ? 'border-l-2 border-violet-600 bg-violet-600/10'
                        : 'hover:bg-white/8'
                    }`}
